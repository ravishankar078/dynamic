...
export function contextAnalyzer(
  ...
  const senderKnown = knownCorrespondents.some(
    (c) => senderLower.includes(c) || c.includes(senderLower.split('@')[0])
  );

  const senderRelationship: ContextAnalysisResult['senderRelationship'] = senderKnown
    ? 'known'
  ...
  let contextVerdict: ContextAnalysisResult['contextVerdict'] = 'NORMAL';
