...
// Enkrypt finding passed into fusion
...
// Qdrant match passed into fusion
...
export function evidenceFusion(
  ...
  enkryptFindings: EnkryptFinding[] = [],     // NEW: Enkrypt AI security evidence
  qdrantMatches: QdrantMatch[] = []           // NEW: Qdrant semantic evidence
): EvidenceFusionResult {
  const positiveRiskEvidence: EvidenceItem[] = [];
  const legitimacyEvidence: EvidenceItem[] = [];
  ...
  const totalWeight = [...positiveRiskEvidence, ...legitimacyEvidence].reduce(
  ...
  const confidence =
    Math.round(
      Math.min(0.95, 0.4 + dominance * 0.3 + (Math.min(totalEvidence, 8) / 8) * 0.25) * 100
    ) / 100;
