...
// Qdrant evidence input — top matches from semantic search
...
// Enkrypt evidence input — AI security gate findings
...
export function calculateRisk(
  ...
  threatMemory: ThreatMemoryResult,
  promptInjection: PromptInjectionResult,
  qdrantMatches: QdrantEvidenceInput[] = [],    // NEW: Qdrant semantic evidence
  enkryptFindings: EnkryptEvidenceInput[] = []   // NEW: Enkrypt AI security evidence
): RiskEngineResult {
  let trust = 50;
  const ledger: TrustAdjustment[] = [];

  function adjust(factor: string, amount: number, reason: string) {
    trust += amount;
    ledger.push({
      factor,
      adjustment: amount,
  ...
  for (const finding of enkryptFindings) {
    if (finding.confidence >= 0.75) {
      adjust(
        'AI Security Gate',
        -35,
        `Enkrypt: ${finding.type} detected at ${Math.round(finding.confidence * 100)}% confidence — ${finding.detail}`
      );
    } else if (finding.confidence >= 0.5) {
      adjust(
        'AI Security Warning',
        -15,
        `Enkrypt: Possible ${finding.type} detected at ${Math.round(finding.confidence * 100)}% confidence`
      );
    }
  }
  ...
  if (threatMemory.matchCount > 0 && threatMemory.highestThreatLevel !== 'none') {
    adjust(
      'Local Threat Match',
      -20,
      `Message matches known threat pattern: ${threatMemory.matches[0]?.category}`
    );
  }
  ...
  if (qdrantMatches.length > 0) {
    ...
    if (topMatch.score >= 0.75) {
      ...
      adjust(
        ...
        intentMatch ? -15 : -8,
        `Semantic similarity ${Math.round(topMatch.score * 100)}% to known ${topMatch.attackType}${intentMatch ? ' (intent confirmed)' : ' (different intent)'}`
      );
    } else if (topMatch.score >= 0.55) {
      adjust(
        'Qdrant Partial Match',
        -5,
        `Partial semantic similarity ${Math.round(topMatch.score * 100)}% to ${topMatch.attackType} — supporting evidence`
      );
    }
