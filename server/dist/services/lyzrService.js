...
exports.initLyzr = initLyzr;
exports.orchestrateTools = orchestrateTools;
...
async function orchestrateTools(params) {
...
{"tools": ["tool1", "tool2", ...], "reasoning": "one sentence explanation"}`;
    try {
        const response = await fetch(`https://agent-prod.studio.lyzr.ai/v3/agents/${agentId}/chat`, {
            method: 'POST',
            headers: {
...
function buildLocalToolPlan(params) {
    ...
    if (params.hasURLs)
        tools.push('analyzeURL');
    if (params.hasEmployeeContext)
        tools.push('contextAnalyzer');
    tools.push('qdrantThreatMemory');
    const reasons = [];
    if (!params.hasURLs)
        reasons.push('URL analysis skipped — no URLs in message');
    if (!params.hasEmployeeContext)
        reasons.push('Context analysis skipped — no employee context provided');
    if (params.enkryptFlagged || params.injectionDetected)
        reasons.push('Injection detection prioritized');
    return {
...
async function invokeAgent(message, context = {}) {
    if (!isConfigured) {
...
//# sourceMappingURL=lyzrService.js.map