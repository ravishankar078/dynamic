...
function checkHomoglyphs(hostname) {
    ...
    if (/(?:micr0|g00g|amaz0n|paypa1|app1e|faceb00k|netf1ix)/.test(hostname))
        return true;
    return false;
}
function detectClaimedBrand(hostname, fullText) {
    ...
    if (/microsoft|office|outlook/i.test(hostname))
        return 'Microsoft';
    if (/google|gmail/i.test(hostname))
        return 'Google';
    if (/apple|icloud/i.test(hostname))
        return 'Apple';
    if (/amazon|aws/i.test(hostname))
        return 'Amazon';
    if (/paypal/i.test(hostname))
        return 'PayPal';
    if (/netflix/i.test(hostname))
        return 'Netflix';
    return null;
}
function checkDomainMismatch(hostname, claimedBrand) {
    if (!claimedBrand)
        return false;
    const legitimateDomains = KNOWN_BRANDS[claimedBrand] || [];
    // Check if hostname is or ends with any legitimate domain
    return !legitimateDomains.some((d) => hostname === d || hostname.endsWith('.' + d));
}
function analyzeOneURL(urlStr, messageText) {
