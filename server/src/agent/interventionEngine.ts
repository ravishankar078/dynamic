...
export function determineIntervention(
  ...
  const severities: Record<InterventionType, InterventionResult['severity']> = {
    ALLOW: 'info',
    WARN: 'warning',
    QUARANTINE: 'danger',
    BLOCK: 'critical',
...
function generateSafeAlternative(intent: string, signals: string[]): string {
  switch (intent) {
    case 'ENTER_PASSWORD':
...
function generateConsequences(intent: string, signals: string[], hasURLs: boolean): string {
  switch (intent) {
    case 'ENTER_PASSWORD':
...
function generateRecommendedSteps(action: InterventionType, intent: string, signals: string[]): string[] {
  const steps: string[] = [];
